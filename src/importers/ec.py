...
class ECImporter(importer.ImporterProtocol):
    def __init__(
        ...
        self.iban = helpers._format_iban(iban)
    ...
    def identify(self, file_):
        with open(file_.name, encoding=self.file_encoding) as fd:
            ...
            for line in reader:
                key, value, *_ = line
                if key == 'IBAN' and helpers._format_iban(value) != self.iban:
                    return False
    ...
    def extract(self, file_, existing_entries=None):
        ...
        with open(file_.name, encoding=self.file_encoding) as fd:
            ...
            for line in reader:
                ...
                self._line_index += 1
                if key == 'IBAN':
                    if helpers._format_iban(values[0]) != self.iban:
                        raise helpers.InvalidFormatError()
                elif key == 'Bank':
            ...
            def balance_assertion(transaction, opening=False, closing=False):
                ...
                line = transaction[1]
                balance = helpers._format_number_de(line['Saldo'])
                ...
                if opening:
                    ...
                    if line['Währung_1'] != line['Währung_2']:
                        ...
                        return []
                    balance -= helpers._format_number_de(line['Betrag'])
